...
def get_ai_service(settings: Settings = Depends(get_settings)) -> GeminiService:
...
def generate_deck(request: GenerateDeckRequest, service: GeminiService = Depends(get_ai_service)) -> GenerateDeckResponse:
    return service.generate_deck(request)


@app.post("/internal/ai/generate-slide", response_model=Slide)
def generate_slide(request: GenerateSlideRequest, service: GeminiService = Depends(get_ai_service)) -> Slide:
